...
export default function History() {
  ...
  const stats = [
    ...
    { name: 'Mon', value: completion['2 days ago'] },
        ...
        <div className="space-y-3">
          {stats.map((day) => (
            <div key={day.name} className="flex items-center">
              <span className="w-20 text-sm font-medium text-gray-600">{day.name}</span>
              <div className="flex-1 h-6 bg-gray-200 rounded-full"><div className="h-6 bg-blue-500 rounded-full" style={{ width: `${day.value * 100}%` }} /></div>
              <span className="w-12 ml-3 text-sm font-medium text-right text-gray-600">{Math.round(day.value * 100)}%</span>
            </div>
          ))}
        </div>
      </div>
