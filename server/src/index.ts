...
app.post('/api/users/:userId/schedule', (req, res) => {
  ...
  if (newSchedule.daysOfWeek.includes(dayOfWeek)) {
     events.push({ id: `evt_${Date.now()}_1`, scheduleId: newSchedule.id, pingTime: '09:00', status: null, dayOfWeek }, { id: `evt_${Date.now()}_2`, scheduleId: newSchedule.id, pingTime: '12:00', status: null, dayOfWeek }, { id: `evt_${Date.now()}_3`, scheduleId: newSchedule.id, pingTime: '15:00', status: null, dayOfWeek });
  }
